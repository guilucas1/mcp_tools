...
# Script to debug and fix the Claude Thread Continuity MCP Server

echo "===== DEBUGGING CLAUDE THREAD CONTINUITY SERVER ====="

# Copy our custom server.py to the current directory
echo "Creating custom server.py file..."
cat > ./custom-server.py << 'EOF'
#!/usr/bin/env python3
"""
...
# Create a debug Dockerfile that uses our custom server.py
echo "Creating debug Dockerfile..."
cat > ./Dockerfile.thread-continuity.debug << 'EOF'
# ============================================
...
# Build with our debug Dockerfile
echo "Building with debug Dockerfile..."
docker-compose build --no-cache -f Dockerfile.thread-continuity.debug thread-continuity

# Start the container
echo "Starting container with our custom server..."
...
echo "Container logs:"
docker-compose logs thread-continuity

# Try accessing the health endpoint
echo "Testing health endpoint:"
curl -v http://localhost:8002/v1/health

echo "===== DEBUG COMPLETE ====="
echo "If the server is still not working, check the logs for more information."